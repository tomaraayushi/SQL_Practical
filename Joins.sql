...
--Inner Join/ Join(USING(same_col_name in both table))
SELECT *
...
--find missing values in the rows by using NULL
SELECT *
FROM district_2020 LEFT JOIN district_2035
ON district_2020.id = district_2035.id
...
--Selecting Specific COlumn in Join
SELECT 
	district_2020.id,
...
INSERT INTO district_2020_enrollment VALUES (1, 360),
				    						(2, 1001),
											(5, 450),
											(6, 927);

INSERT INTO district_2020_grades VALUES (1, 'K-3'),
										(2, '9-12'),
										(5, '6-8'),
										(6, '9-12');
...
SELECT * FROM district_2020
UNION ALL --all rows with duplicates included
SELECT * FROM district_2035
ORDER BY id;
...
EXCEPT --rows from first query only

SELECT * FROM district_2035
ORDER BY id;

--Math on Joined columns
CREATE TABLE us_counties_pop_est_2010(
	...
	state_name text,
	county_name text,
	estimate_base_2010 integer,
	CONSTRAINT counties_2010_key PRIMARY KEY (state_fips, county_fips)
);

COPY us_counties_pop_est_2010
FROM 'D:\SQL_practice\practical-sql-2-main\Chapter_07\us_counties_pop_est_2010.csv'
...
SELECT
	...
	c2010.estimate_base_2010 AS pop_2010,
	c2019.pop_est_2019 - c2010.estimate_base_2010 AS raw_change,
	round( (c2019.pop_est_2019::numeric - c2010.estimate_base_2010)/ 
		  	c2010.estimate_base_2010 * 100, 1) AS pct_change
